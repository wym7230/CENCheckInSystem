...
router.route('/events/:id')
    .get(function (req, res) {
        Event.findById(req.params.id)
            .populate({path: 'attended_officers._id', select: '_id + name'})
...
router.route('/events/:event_id/checkin/:id')
    .put(function (req, res) {
...
router.put('/events/:event_id/vacate/:id', function (req, res) {
    Officer.findById(req.params.id, '_id', function (err, officer) {
        if(err) return res.send(err);
        Event.update({_id: req.params.event_id},
            {$pull: {unattended_officers: officer._id}},
                ...
                if(pullRaw.nModified != 0){
                    Event.update({_id: req.params.event_id},
                        ...
                        function (err, addRaw) {
                            ...
                            if(addRaw.nModified != 0){
                                Officer.update({_id: req.params.id},
                                    {$inc: {asked_for_leave_numbers: 1, unattended_numbers: -1}},
                                    function (err, raw) {
                                        if(err) return console.log(err);
                                        return console.log("update officer attendance: ", raw);
                                    })
                            }else{
                                return console.log("Failed to set this user vacate", err)
                            }
                        });
                }else {
...
router.put('/events/:event_id/unvacate/:id', function (req, res) {
    Officer.findById(req.params.id, '_id', function (err, officer) {
        if(err) return res.send(err);
        Event.update({_id: req.params.event_id},
            {$pull: {asked_for_leave_officers: officer._id}},
            function (err, pullRaw) {
                if(err) return res.send(err);
                console.log(pullRaw);
                if(pullRaw.nModified != 0){
                    Event.update({_id: req.params.event_id},
                        ...
                        function (err, addRaw) {
                            ...
                            if(addRaw.nModified != 0){
                                ...
                                return console.log("Failed to unset this user vacate", err)
                            }
                        });
                }else {
                    return console.log("Failed to unset this user vacate");
                }
            });
        return res.json("Successful");
    })
